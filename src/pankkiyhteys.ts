...
export class NordeaCertService extends SoapClient implements app.CertService {
  ...
  constructor(username: string, environment: app.Environment = app.Environment.TEST) {
    super()
  ...
  async addIntermediaryCertificates(trustStore: TrustStore) {
    ...
    // Cert service envelopes are not signed.
    const response = await this.makeSoapRequest(NordeaCertService.getEndpoint(), {
      getServiceCertificatesin: {
        '@xmlns': NordeaCertService.getEndpoint(),
        RequestHeader: {
          SenderId: this.username,
          RequestId: this.requestId(),
          Timestamp: this.formatTime(new Date())
...
const nordeaXmlns = 'http://bxd.fi/xmldata/'

export class Nordea extends app.Client {
  certService: NordeaCertService
  ...
  constructor(
    ...
    const endpoint = Nordea.getEndpoint(environment)
    const bic = 'NDEAFIHH'
